...
void test_capture_completes_bounded(void) {
    ...
    char logpath[256];
#ifdef _WIN32
    /* Windows: system() uses cmd.exe. Windows carrom_arena builds with WGL/OpenGL
     * and can create a hidden window natively via FLAG_WINDOW_HIDDEN (Gap 2 renderer).
     * However, GitHub windows-2022 runners lack GPU, so headless WGL fails.
     * Skip capture test on Windows CI until GPU runners are available. */
    const char* ci = getenv("CI");
    const char* runner_os = getenv("RUNNER_OS");
    if (ci && runner_os && strcmp(runner_os, "Windows") == 0) {
        // In Windows CI without GPU: skip test gracefully
        return;
    }
    /* Local Windows with GPU: run normally */
