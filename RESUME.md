...
2. Re-verify reachability: Linux `sanyalnet@192.168.4.76`, Windows 10 `sanyalnet@192.168.4.75`, Windows 11 `vagab@192.168.4.103`, macOS `rumtuk@192.168.4.77` (bonus, not required). Password `harryseldon` everywhere; SSH keys are also installed (`~/.ssh/id_ed25519` on the Linux box) for passwordless cross-machine access.
